...
def filter_ui_tree(input_path, output_path, image_filename):
    ...
    def traverse(node, clip_bounds):
        ...
        # Note: In some web cases, children can overflow. But for standard hidden filtering, 
        # strict clipping is safer.
        if not visible_rect or get_area(visible_rect) < 100: # 10x10 = 100
            # If the node itself is small, it might still contain visible children if overflow is allowed.
            # However, usually 1x1 parents hide children. We'll enforce this.
            return
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(base_dir, "dataset_cropped/github/filtered.json")
