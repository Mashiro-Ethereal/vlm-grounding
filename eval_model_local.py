...
MODEL_NAME = "ByteDance-Seed/UI-TARS-1.5-7B"  # 指定测试模型

# 2. 数据集设置
DATASET_PATH = "GUI_Grounding_Benchmark/test.jsonl"  # 你的本地数据集路径
safe_model_name = MODEL_NAME.replace('/', '_')
OUTPUT_FILE = f"eval_results_{safe_model_name}.jsonl" # 评测结果保存路径
...
# 4. 并发设置
MAX_WORKERS = 2  # 根据 API 速率限制调整并发数
# ===========================================

# client = OpenAI(base_url=BASE_URL, api_key=API_KEY)
client = OpenAI(
    base_url="http://localhost:8000/v1",
    api_key="EMPTY"  # 本地部署通常不需要 Key，但在 client 初始化时必须填一个占位符
)
